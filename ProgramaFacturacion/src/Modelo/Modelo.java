...
public class Modelo {
    ...
    public void generarArticulo(String nombre, String precio){
        ...
        String precioFormateado = String.format("%.2f", precioInstancia);
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("articulos.txt", true))) {
            ...
            writer.newLine(); // NUEVA LINEA PARA CADA ARTÍCULO
    ...
    public void generarFactura(String fecha, String dniCliente,
        ...
        Factura factura = new Factura(fecha, cliente, getIVA());
            ...
            for (LineaFacturas linea : lineas) {
                ...
                writer.write(
                        factura.getNumeroFactura() + ";" +
                                ...
                                String.format("%.2f",linea.calcularCantidadPrecio())
                );
                writer.newLine();
            }
        } catch (IOException e) {
    ...
    public void persistencia() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("iva.txt"))) {

            String linea = reader.readLine();
            if (linea != null && !linea.isBlank()) {
                int ivaLeido = Integer.parseInt(linea.trim());
                this.iva = ivaLeido;
                System.out.println("IVA cargado desde el fichero: " + ivaLeido + "%");
            } else {
                System.out.println("iva.txt vacío. Se mantiene el IVA actual: " + this.iva);
            }

        } catch (Exception e) {
            System.out.println("Error al cargar IVA: " + e.getMessage());
        }
    }
